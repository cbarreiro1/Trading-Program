...
def is_macd_line_over_signal(price_history, symbol):
...
def is_macd_line_over_zero(price_history, symbol):
    macd_line = price_history[symbol]['MACD Line']
    return macd_line.iloc[-1] > 0

# Function to check if the MACD has crossed over the Signal line
def has_macd_crossed_over_signal(price_history, symbol):
    macd_line = price_history[symbol]['MACD Line']
    signal_line = price_history[symbol]['Signal Line']
...
def has_macd_crossed_over_zero(price_history, symbol):
    macd_line = price_history[symbol]['MACD Line']
